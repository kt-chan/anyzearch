...
const TRANSLATIONS = {
  ...
  welcomeMessage: {
    part1:
      "欢迎使用 AnyZearch，这是由 iteasoft labs 开发的开源 AI 工具，可以将任何东西转换为您可以查询和聊天的训练有素的聊天机器人。",
    part2:
      "AnyZearch 是将强大的 AI 产品（如 OpenAi、GPT-4、LangChain、PineconeDB、ChromaDB 等）整合在一个整洁的包中而无需繁琐操作的最简单方法，可以将您的生产力提高 100 倍。",
    part3:
      "AnyZearch 可以完全在您的本地计算机上运行，几乎没有开销，您甚至不会注意到它的存在！无需 GPU。也可以进行云端和本地安装。\nAI 工具生态系统每天都在变得更强大。AnyZearch 使其易于使用。",
  ...
  llm: {
    title: "LLM 偏好",
    description:
      "这些是您首选的 LLM 聊天和嵌入提供商的凭据和设置。重要的是，这些密钥是最新的和正确的，否则 AnyZearch 将无法正常运行。",
    provider: "LLM 提供商",
  },
  ...
  embedding: {
    ...
    "desc-end":
      "嵌入是将文本转换为矢量的过程。需要这些凭据才能将您的文件和提示转换为 AnyZearch 可以用来处理的格式。",
    provider: {
      title: "嵌入引擎提供商",
      description: "使用 AnyZearch 的本机嵌入引擎时不需要设置。",
    },
  },
  ...
  multi: {
    ...
    password: {
      title: "密码保护",
      description:
        "用密码保护您的AnyZearch实例。如果您忘记了密码，那么没有恢复方法，所以请确保保存这个密码。",
    },
    instance: {
      title: "实例密码保护",
      description:
  ...
  privacy: {
    title: "隐私和数据处理",
    description:
      "这是您对如何处理连接的第三方提供商和AnyZearch的数据的配置。",
    llm: "LLM选择",
    embedding: "嵌入偏好",
    vector: "向量数据库",
    anonymous: "启用匿名遥测",
